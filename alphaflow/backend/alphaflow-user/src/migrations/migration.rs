...
use alphaflow_error::AlphaflowResult;
use alphaflow_sqlite::kv::KVStorePreferences;
use alphaflow_sqlite::schema::user_data_migration_records;
use alphaflow_sqlite::ConnectionPool;
use alphaflow_user_pub::entities::Authenticator;
use alphaflow_user_pub::session::Session;
use semver::Version;
use tracing::info;

/// Store the version that user first time install Alphaflow. For old user, this value will be set
...
pub trait UserDataMigration {
  ...
  fn name(&self) -> &str;
  // The user's initial installed version is None if they were using an Alphaflow version lower than 0.7.3
...
fn get_all_records(conn: &mut SqliteConnection) -> AlphaflowResult<Vec<UserDataMigrationRecord>> {
  Ok(
