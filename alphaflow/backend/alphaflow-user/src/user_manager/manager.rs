use collab_integrate::collab_builder::AlphaFlowCollabBuilder;
use collab_integrate::CollabKVDB;
use alphaflow_error::{internal_error, ErrorCode, AlphaflowResult};

use arc_swap::ArcSwapOption;
use collab::lock::RwLock;
use collab_user::core::UserAwareness;
use dashmap::DashMap;
use alphaflow_server_pub::AuthenticatorType;
use alphaflow_sqlite::kv::KVStorePreferences;
use alphaflow_sqlite::schema::user_table;
use alphaflow_sqlite::ConnectionPool;
use alphaflow_sqlite::{query_dsl::*, DBConnection, ExpressionMethods};
use alphaflow_user_pub::cloud::{UserCloudServiceProvider, UserUpdate};
use alphaflow_user_pub::entities::*;
use alphaflow_user_pub::workspace_service::UserWorkspaceService;
use semver::Version;
...
use crate::{errors::AlphaflowError, notification::*};
...
pub struct UserManager {
  ...
  pub(crate) collab_builder: Weak<AlphaFlowCollabBuilder>,
...
impl UserManager {
  pub fn new(
    ...
    collab_builder: Weak<AlphaFlowCollabBuilder>,
  ...
  pub async fn init_with_callback<C: UserStatusCallback + 'static, I: UserReminder>(
    ...
    *self.collab_interact.write().await = Arc::new(collab_interact);
    ...
    if let Ok(session) = self.get_session() {
      ...
      // Currently, only the AlphaFlow cloud using token to init the client api.
      // TODO(nathan): using trait to separate the init process for different cloud service
      if user.authenticator.is_alphaflow_cloud() {
  ...
  pub fn get_session(&self) -> AlphaflowResult<Arc<Session>> {
    self.authenticate_user.get_session()
  }

  pub fn db_connection(&self, uid: i64) -> Result<DBConnection, AlphaflowError> {
    self.authenticate_user.database.get_connection(uid)
  }

  pub fn db_pool(&self, uid: i64) -> Result<Arc<ConnectionPool>, AlphaflowError> {
    self.authenticate_user.database.get_pool(uid)
  }

  pub fn get_collab_db(&self, uid: i64) -> Result<Weak<CollabKVDB>, AlphaflowError> {
    self
      .authenticate_user
      .database
      .get_collab_db(uid)
  ...
  ) -> Result<UserProfile, AlphaflowError> {
  ...
  pub async fn resume_sign_up(&self) -> Result<(), AlphaflowError> {
    let UserAuthProcess {
      user_profile,
      migration_user,
      response,
  ...
  #[tracing::instrument(level = "info", skip(self))]
  pub async fn sign_out(&self) -> Result<(), AlphaflowError> {
    if let Ok(session) = self.get_session() {
      sign_out(
        &self.cloud_services,
        &session,
  ...
  #[tracing::instrument(level = "info", skip(self))]
  pub async fn delete_account(&self) -> Result<(), AlphaflowError> {
    self
      .cloud_services
      .get_user_service()?
      .delete_account()
  ...
  pub async fn init_user(&self) -> Result<(), AlphaflowError> {
    Ok(())
  }
  ...
  pub async fn get_user_profile_from_disk(&self, uid: i64) -> Result<UserProfile, AlphaflowError> {
    select_user_profile(uid, self.db_connection(uid)?)
  }

  #[tracing::instrument(level = "info", skip_all, err)]
  pub async fn refresh_user_profile(&self, old_user_profile: &UserProfile) -> AlphaflowResult<()> {
    ...
    let result: Result<UserProfile, AlphaflowError> = self
      .cloud_services
      .get_user_service()?
  ...
  pub fn user_setting(&self) -> Result<UserSettingPB, AlphaflowError> {
    let session = self.get_session()?;
  ...
  pub fn user_id(&self) -> Result<i64, AlphaflowError> {
    Ok(self.get_session()?.user_id)
  }

  pub fn workspace_id(&self) -> Result<String, AlphaflowError> {
    let session = self.get_session()?;
    Ok(session.user_workspace.id.clone())
  }

  pub fn token(&self) -> Result<Option<String>, AlphaflowError> {
    Ok(None)
  }
  ...
  async fn save_user(&self, uid: i64, user: UserTable) -> Result<(), AlphaflowError> {
  ...
  pub(crate) async fn sign_in_with_magic_link(
    ...
    self
      .cloud_services
      .set_user_authenticator(&Authenticator::AlphaFlowCloud);
    let auth_service = self.cloud_services.get_user_service()?;
    auth_service
  ...
  async fn handler_user_update(&self, user_update: UserUpdate) -> AlphaflowResult<()> {
    let session = self.get_session()?;
  ...
  async fn migrate_anon_user_data_to_cloud(
    ...
    if authenticator == &Authenticator::AlphaFlowCloud {
      self
        .migration_anon_user_on_alphaflow_cloud_sign_up(old_user, &old_collab_db)
        .await?;
    }
...
fn current_authenticator() -> Authenticator {
  match AuthenticatorType::from_env() {
    ...
    AuthenticatorType::AlphaFlowCloud => Authenticator::AlphaFlowCloud,
  }
}
...
#[instrument(level = "info", skip_all, err)]
fn save_user_token(uid: i64, conn: DBConnection, token: String) -> AlphaflowResult<()> {
...
#[instrument(level = "info", skip_all, err)]
fn remove_user_token(uid: i64, mut conn: DBConnection) -> AlphaflowResult<()> {
...
pub(crate) fn run_collab_data_migration(
  ...
  match UserLocalDataMigration::new(session.clone(), collab_db, sqlite_pool, kv).run(
    ...
    Err(e) => error!("[AlphaflowData]:User data migration failed: {:?}", e),
  }
}
