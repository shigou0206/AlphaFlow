...
use collab_integrate::collab_builder::{
  AlphaFlowCollabBuilder, CollabBuilderConfig, CollabPersistenceImpl,
};
...
impl UserManager {
  ...
  /// - May return errors of type `AlphaflowError` if any issues arise during the process.
  ///
  pub async fn add_reminder(&self, reminder_pb: ReminderPB) -> AlphaflowResult<()> {
  ...
  ///
  pub async fn remove_reminder(&self, reminder_id: &str) -> AlphaflowResult<()> {
    self
      .mut_awareness(|user_awareness| {
  ...
  ///
  pub async fn update_reminder(&self, reminder_pb: ReminderPB) -> AlphaflowResult<()> {
    let reminder = Reminder::from(reminder_pb);
    self
      .mut_awareness(|user_awareness| {
  ...
  ) -> Result<Arc<RwLock<UserAwareness>>, AlphaflowError> {
    let collab_builder = collab_builder.upgrade().ok_or(AlphaflowError::new(
      ErrorCode::Internal,
  ...
  async fn mut_awareness<F, Output>(&self, f: F) -> AlphaflowResult<Output>
  where
