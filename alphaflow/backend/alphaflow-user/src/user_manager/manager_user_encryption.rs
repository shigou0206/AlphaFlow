...
impl UserManager {
  ...
  pub fn generate_encryption_sign(&self, uid: i64, encrypt_secret: &str) -> AlphaflowResult<String> {
    let encrypt_sign = encrypt_text(uid.to_string(), encrypt_secret)?;
    Ok(encrypt_sign)
  }

  pub fn check_encryption_sign(&self, uid: i64, encrypt_sign: &str) -> AlphaflowResult<()> {
    ...
    let encrypt_secret = get_encrypt_secret(uid, &store_preference).ok_or(AlphaflowError::new(
      ErrorCode::Internal,
  ...
  pub fn check_encryption_sign_with_secret(
    ...
    let decrypt_str = decrypt_text(encrypt_sign, encryption_secret)
      .map_err(|_| AlphaflowError::new(ErrorCode::InvalidEncryptSecret, "Invalid decryption secret"))?;
