...
use alphaflow_error::{ErrorCode, AlphaflowError, AlphaflowResult};
use alphaflow_folder_pub::entities::{ImportFrom, ImportedCollabData, ImportedFolderData};
use alphaflow_sqlite::schema::user_workspace_table;
...
impl UserManager {
  /// Import alphaflow data from the given path.
  ...
  pub(crate) async fn perform_import(&self, imported_folder: ImportedFolder) -> AlphaflowResult<()> {
    ...
    let import_data = tokio::task::spawn_blocking(move || {
      generate_import_data(&cloned_current_session, &user_collab_db, imported_folder)
        .map_err(|err| AlphaflowError::new(ErrorCode::AlphaFlowDataFolderImportError, err.to_string()))
    })
    .await??;

    info!(
      "[AlphaFlowData]: upload {} document, {} database, {}, rows",
  ...
  async fn upload_collab_data(
    ...
    let user_collab_db = self
      .get_collab_db(current_session.user_id)?
      .upgrade()
      .ok_or_else(|| AlphaflowError::new(ErrorCode::Internal, "Collab db not found"))?;
  ...
  pub async fn migration_anon_user_on_alphaflow_cloud_sign_up(
    &self,
    old_user: &AnonUser,
  ...
  pub async fn open_workspace(&self, workspace_id: &str) -> AlphaflowResult<()> {
  ...
  pub async fn add_workspace(&self, workspace_name: &str) -> AlphaflowResult<UserWorkspace> {
  ...
  pub async fn patch_workspace(
    ...
    let mut user_workspace = match self.get_user_workspace(uid, workspace_id) {
      ...
      None => {
        return Err(AlphaflowError::new(ErrorCode::RecordNotFound, format!(
  ...
  #[instrument(level = "info", skip(self), err)]
  pub async fn leave_workspace(&self, workspace_id: &str) -> AlphaflowResult<()> {
  ...
  #[instrument(level = "info", skip(self), err)]
  pub async fn delete_workspace(&self, workspace_id: &str) -> AlphaflowResult<()> {
  ...
  pub async fn list_pending_workspace_invitations(&self) -> AlphaflowResult<Vec<WorkspaceInvitation>> {
  ...
  pub async fn accept_workspace_invitation(&self, invite_id: String) -> AlphaflowResult<()> {
    self
      .cloud_services
      .get_user_service()?
  ...
  pub async fn get_workspace_members(
    &self,
    workspace_id: String,
  ) -> AlphaflowResult<Vec<WorkspaceMember>> {
    let members = self
      .cloud_services
      .get_user_service()?
  ...
  pub async fn get_workspace_member(
    ...
    workspace_id: String,
    uid: i64,
  ) -> AlphaflowResult<WorkspaceMember> {
    let member = self
      .cloud_services
      .get_user_service()?
  ...
  pub async fn get_all_user_workspaces(&self, uid: i64) -> AlphaflowResult<Vec<UserWorkspace>> {
    let conn = self.db_connection(uid)?;
  ...
  pub async fn reset_workspace(&self, reset: ResetWorkspacePB) -> AlphaflowResult<()> {
  ...
  pub async fn get_workspace_subscription_info(
    &self,
    workspace_id: String,
  ) -> AlphaflowResult<WorkspaceSubscriptionInfoPB> {
  ...
  #[instrument(level = "info", skip(self), err)]
  pub async fn get_subscription_plan_details(&self) -> AlphaflowResult<Vec<SubscriptionPlanDetail>> {
  ...
  pub async fn get_workspace_usage(
    &self,
    workspace_id: String,
  ) -> AlphaflowResult<WorkspaceUsageAndLimit> {
  ...
  #[instrument(level = "info", skip(self), err)]
  pub async fn get_billing_portal_url(&self) -> AlphaflowResult<String> {
    let url = self
      .cloud_services
      .get_user_service()?
  ...
  pub async fn get_workspace_settings(&self, workspace_id: &str) -> AlphaflowResult<UseAISettingPB> {
    let cloud_service = self.cloud_services.get_user_service()?;
  ...
  pub async fn get_workspace_member_info(&self, uid: i64) -> AlphaflowResult<WorkspaceMember> {
...
pub fn delete_user_workspaces(mut conn: DBConnection, workspace_id: &str) -> AlphaflowResult<()> {
  let n = conn.immediate_transaction(|conn| {
    let rows_affected: usize =
      diesel::delete(user_workspace_table::table.filter(user_workspace_table::id.eq(workspace_id)))
        .execute(conn)?;
    Ok::<usize, AlphaflowError>(rows_affected)
  })?;
  if n != 1 {
