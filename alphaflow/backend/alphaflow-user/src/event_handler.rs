...
use alphaflow_user_pub::cloud::UserCloudConfig;
...
fn upgrade_manager(manager: AFPluginState<Weak<UserManager>>) -> AlphaflowResult<Arc<UserManager>> {
  let manager = manager
    .upgrade()
    .ok_or(AlphaflowError::internal().with_context("The user session is already drop"))?;
  Ok(manager)
}

fn upgrade_store_preferences(
  store: AFPluginState<Weak<KVStorePreferences>>,
) -> AlphaflowResult<Arc<KVStorePreferences>> {
  let store = store
    .upgrade()
    .ok_or(AlphaflowError::internal().with_context("The store preferences is already drop"))?;
  Ok(store)
}
...
pub async fn sign_in_with_email_password_handler(
  ...
  ) -> DataResult<UserProfilePB, AlphaflowError> {
...
#[tracing::instrument(level = "debug", skip(manager))]
pub async fn sign_out_handler(manager: AFPluginState<Weak<UserManager>>) -> Result<(), AlphaflowError> {
...
pub async fn get_appearance_setting(
  store_preferences: AFPluginState<Weak<KVStorePreferences>>,
) -> DataResult<AppearanceSettingsPB, AlphaflowError> {
  let store_preferences = upgrade_store_preferences(store_preferences)?;
...
pub async fn get_date_time_settings(
  store_preferences: AFPluginState<Weak<KVStorePreferences>>,
) -> DataResult<DateTimeSettingsPB, AlphaflowError> {
  let store_preferences = upgrade_store_preferences(store_preferences)?;
...
pub async fn get_notification_settings(
  store_preferences: AFPluginState<Weak<KVStorePreferences>>,
) -> DataResult<NotificationSettingsPB, AlphaflowError> {
  let store_preferences = upgrade_store_preferences(store_preferences)?;
...
#[tracing::instrument(level = "debug", skip_all, err)]
pub async fn import_alphaflow_data_folder_handler(
  data: AFPluginData<ImportAlphaFlowDataPB>,
  manager: AFPluginState<Weak<UserManager>>,
) -> Result<(), AlphaflowError> {
...
pub async fn get_user_setting(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<UserSettingPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn gen_sign_in_url_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<SignInUrlPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
  let params = data.into_inner();
  let authenticator: Authenticator = params.authenticator.into();
...
pub async fn sign_in_with_provider_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<OauthProviderDataPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn check_encrypt_secret_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<UserEncryptionConfigurationPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
  let uid = manager.get_session()?.user_id;
...
pub async fn set_cloud_config_handler(
  ...
  let mut config = get_cloud_config(session.user_id, &store_preferences)
    .ok_or(AlphaflowError::internal().with_context("Can't find any cloud config"))?;
...
pub async fn get_cloud_config_handler(
  manager: AFPluginState<Weak<UserManager>>,
  store_preferences: AFPluginState<Weak<KVStorePreferences>>,
) -> DataResult<CloudSettingPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
  let session = manager.get_session()?;
  let store_preferences = upgrade_store_preferences(store_preferences)?;
...
pub async fn get_all_workspace_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<RepeatedUserWorkspacePB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
  let uid = manager.get_session()?.user_id;
...
pub async fn get_all_reminder_event_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<RepeatedReminderPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn get_workspace_members_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<RepeatedWorkspaceMemberPB, AlphaflowError> {
  let data = data.try_into_inner()?;
  let manager = upgrade_manager(manager)?;
  let members = manager
...
pub async fn create_workspace_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<UserWorkspacePB, AlphaflowError> {
  let data = data.try_into_inner()?;
  let manager = upgrade_manager(manager)?;
...
pub async fn list_workspace_invitations_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<RepeatedWorkspaceInvitationPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn subscribe_workspace_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<PaymentLinkPB, AlphaflowError> {
...
pub async fn get_workspace_subscription_info_handler(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<WorkspaceSubscriptionInfoPB, AlphaflowError> {
  let params = params.try_into_inner()?;
  let manager = upgrade_manager(manager)?;
  let subs = manager
...
pub async fn get_workspace_usage_handler(
  param: AFPluginData<UserWorkspaceIdPB>,
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<WorkspaceUsagePB, AlphaflowError> {
  let workspace_id = param.into_inner().workspace_id;
  let manager = upgrade_manager(manager)?;
...
pub async fn get_billing_portal_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<BillingPortalPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn get_subscription_plan_details_handler(
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<RepeatedSubscriptionPlanDetailPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
  let plans = manager
...
pub async fn get_workspace_member_info(
  ...
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<WorkspaceMemberPB, AlphaflowError> {
  let manager = upgrade_manager(manager)?;
...
pub async fn get_workspace_setting(
  params: AFPluginData<UserWorkspaceIdPB>,
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<UseAISettingPB, AlphaflowError> {
  let params = params.try_into_inner()?;
  let manager = upgrade_manager(manager)?;
