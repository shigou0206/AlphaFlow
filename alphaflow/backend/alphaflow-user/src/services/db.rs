...
use dashmap::DashMap;
use alphaflow_error::AlphaflowError;
...
use alphaflow_user_pub::entities::{UserProfile, UserWorkspace};
...
impl UserDB {
  ...
  pub(crate) fn close(&self, user_id: i64) -> Result<(), AlphaflowError> {
  ...
  pub(crate) fn get_connection(&self, user_id: i64) -> Result<DBConnection, AlphaflowError> {
    let conn = self.get_pool(user_id)?.get()?;
    Ok(conn)
  }

  pub(crate) fn get_pool(&self, user_id: i64) -> Result<Arc<ConnectionPool>, AlphaflowError> {
    let pool = self.open_sqlite_db(self.paths.sqlite_db_path(user_id), user_id)?;
    Ok(pool)
  }

  pub(crate) fn get_collab_db(&self, user_id: i64) -> Result<Arc<CollabKVDB>, AlphaflowError> {
  ...
  pub fn open_sqlite_db(
    ...
    user_id: i64,
  ) -> Result<Arc<ConnectionPool>, AlphaflowError> {
    match self.sqlite_map.entry(user_id) {
      ...
      Entry::Vacant(e) => {
        ...
        let db = alphaflow_sqlite::init(&db_path).map_err(|e| {
          AlphaflowError::new(ErrorCode::Internal, format!("open user db failed, {:?}", e))
        })?;
  ...
  pub fn get_user_workspace(
    ...
    pool: &Arc<ConnectionPool>,
    uid: i64,
  ) -> Result<Option<UserWorkspace>, AlphaflowError> {
    let mut conn = pool.get()?;
