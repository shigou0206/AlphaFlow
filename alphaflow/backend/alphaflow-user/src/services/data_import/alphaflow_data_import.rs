...
use alphaflow_error::{AlphaflowError, AlphaflowResult, ErrorCode};
use alphaflow_folder_pub::cloud::gen_view_id;
use alphaflow_folder_pub::entities::{
  ImportFrom, ImportedAlphaflowData, ImportedCollabData, ImportedFolderData,
};
use alphaflow_sqlite::kv::KVStorePreferences;
use alphaflow_user_pub::cloud::{UserCloudService, UserCollabParams};
use alphaflow_user_pub::entities::{user_awareness_object_id, Authenticator};
use alphaflow_user_pub::session::Session;
use rayon::prelude::*;
...
pub(crate) fn prepare_import(
  ...
  info!(
    "[AlphaflowData]:importing data from path: {}, parent_view_id:{:?}",
    path, parent_view_id
  );
  ...
  migrate_session_with_user_uuid("alphaflow_session_cache", &other_store_preferences);
  let imported_session = other_store_preferences
    .get_object::<Session>("alphaflow_session_cache")
    .ok_or(anyhow!(
      "Can't find the session cache in the alphaflow data folder at path: {}",
      path
    ))?;

  let collab_db_path = user_paths.collab_db_path(imported_session.user_id);
  info!("[AlphaflowData]: collab db path: {:?}", collab_db_path);

  let sqlite_db_path = user_paths.sqlite_db_path(imported_session.user_id);
  info!("[AlphaflowData]: sqlite db path: {:?}", sqlite_db_path);

  let imported_sqlite_db = alphaflow_sqlite::init(sqlite_db_path)
    .map_err(|err| anyhow!("[AlphaflowData]: open import collab db failed: {:?}", err))?;
...
/// This path refers to the directory where AlphaFlow stores its data. The directory structure is as follows:
/// root folder:
///   - cache.db
...
pub(crate) fn generate_import_data(
...
) -> anyhow::Result<ImportedAlphaflowData> {
  info!(
    "[AlphaflowData]:importing workspace: {}:{}",
  ...
  let (views, orphan_views) = user_collab_db.with_write_txn(|current_collab_db_write_txn| {
    ...
    info!(
      "[AlphaflowData]: {} has {} collab objects",
      imported_uid,
    ...
    if let Err(err) = mapping_workspace_database_ids(
      ...
      error!("[AlphaflowData]:import workspace database fail: {}", err);
    }
    ...
    info!(
      "[AlphaflowData]: invalid object ids: {:?}",
      invalid_object_ids
    );
    ...
    // Currently, when importing AlphaFlow data, the view which is a space doesn't have a corresponding
    // collab data in disk.
    for view_id in not_exist_parent_view_ids {
      if let Err(err) = create_empty_document_for_view(
        ...
        error!(
          "[AlphaflowData]: create empty document for view failed: {}",
          err
        );
      }
    }
  ...
  let source = match imported_folder.source {
    ImportedSource::ExternalFolder => ImportFrom::AlphaFlowDataFolder,
    ImportedSource::AnonUser => ImportFrom::AnonUser,
  };

  Ok(ImportedAlphaflowData {
    source,
  ...
  PersistenceError: From<W::Error>,
{
  child_views.iter_mut().for_each(|parent_child_views| {
    if parent_child_views.view.parent_view_id != import_container_view_id {
      warn!(
        "[AlphaflowData]: The parent view id of the child views is not the import container view id: {}",
        import_container_view_id
      );
...
fn init_workspace_database(object_id: &str, collab: Collab) -> WorkspaceDatabase {
  match WorkspaceDatabase::open(collab) {
    ...
    Err(err) => {
      error!(
        "[AlphaflowData]:init workspace database body failed: {:?}, create a new one",
        err
      );
  ...
  PersistenceError: From<W::Error>,
{
  ...
  for object_id in all_imported_object_ids.iter() {
    if let Some(database_collab) = imported_collab_by_oid.get_mut(object_id) {
      ...
      {
        error!(
          "[AlphaflowData]: reset inline view id failed for database: {}",
          object_id
        );
        continue;
      }
    ...
    let gen_database_row_collabs = imported_row_ids
      ...
      .filter_map(|imported_row_id| {
        ...
        match old_to_new_id_map.get_exchanged_id(imported_row_id) {
          None => {
            error!(
              "[AlphaflowData]: Can't find the new id for the imported row:{}",
              imported_row_id
            );
            None
          },
    ...
    collab.encode_collab_v1(|collab| collab_type.validate_require_data(collab))
  {
    if let Ok(update) = Update::decode_v1(&encode_collab.doc_state) {
      ...
      if let Err(err) = w_txn.flush_doc(
        ...
        error!(
          "[AlphaflowData]:import collab:{} failed: {:?}",
          new_object_id, err
        );
      }
    }
  ...
  let (state_vector, doc_state) = match collab_type {
    CollabType::Document => {
      ...
      if let Err(err) = replace_document_ref_ids(&mut document, ids_map) {
        error!("[AlphaflowData]: replace document ref ids failed: {}", err);
      }
  ...
  PersistenceError: From<W::Error>,
{
  ...
  imported_collab_db_read_txn
    ...
    .map_err(|err| {
      PersistenceError::Internal(anyhow!(
        "[AlphaflowData]: Can't load the user:{} folder:{}. {}",
        imported_session.user_id,
        imported_session.user_workspace.id,
        err
      ))
  ...
  let imported_folder =
    Folder::open(other_user_id, imported_folder_collab, None).map_err(|err| {
      PersistenceError::Internal(anyhow!("[AlphaflowData]:Can't open folder:{}", err))
    })?;
  ...
  let mut imported_folder_data = imported_folder
    ...
    .ok_or(PersistenceError::Internal(anyhow!(
      "[AlphaflowData]: Can't read the folder data"
    )))?;
  ...
  imported_folder_data.views.retain(|view| {
    ...
    if view.layout.is_database() && !database_view_ids.contains(&new_view_id) {
      error!(
        "[AlphaflowData]: The database view:{} is not in the database view ids",
        view.id
      );
    }
  ...
  let parent_views = views_not_in_trash
    ...
    .flat_map(
      |view_identifier| match all_views_map.remove(&view_identifier.id) {
        None => {
          warn!(
            "[AlphaflowData]: Can't find the view:{} in the all views map",
            view_identifier.id
          );
          None
        },
  ...
  info!(
    "[AlphaflowData]: create orphan views: {:?}",
    all_views_map.keys()
  );
  ...
  info!(
    "[AlphaflowData]: parent views: {}, orphan views: {}, invalid orphan views: {}, views without collab data: {}",
    parent_views.len(),
    orphan_views.len(),
...
) -> Result<(), AlphaflowError> {
  // Only support uploading the collab data when the current server is AlphaFlow Cloud server
  if !user_authenticator.is_alphaflow_cloud() {
    return Ok(());
  }
  ...
  } = collab_data;
  {
    ...
    let object_by_collab_type = tokio::task::spawn_blocking(move || {
      let user_collab_db = user_collab_db.upgrade().ok_or_else(|| {
        AlphaflowError::new(ErrorCode::Internal,
      ...
      event!(
        tracing::Level::DEBUG,
        "[AlphaflowData]:upload database collab data"
      );
      ...
      event!(
        tracing::Level::DEBUG,
        "[AlphaflowData]:upload document collab data"
      );
      object_by_collab_type.insert(
        CollabType::Document,
        load_and_process_collab_data(
      ...
      event!(
        tracing::Level::DEBUG,
        "[AlphaflowData]:upload database row collab data"
      );
      object_by_collab_type.insert(
        CollabType::DatabaseRow,
        load_and_process_collab_data(uid, &cloned_workspace_id, &collab_read, &row_object_ids),
      );
      Ok::<_, AlphaflowError>(object_by_collab_type)
    })
    .await??;
...
async fn batch_create(
    ...
    .await
  {
    Ok(_) => {
      info!(
        "[AlphaflowData]:Batch creating collab objects success, origin payload size: {}",
        size_counter
      );
    },
    Err(err) => {
      error!(
      "[AlphaflowData]:Batch creating collab objects fail, origin payload size: {}, workspace_id:{}, uid: {}, error: {:?}",
        ...
        if updated {
          ...
          document.update_block(block_id, data).map_err(|err| {
            anyhow::Error::msg(format!(
              "[AlphaflowData]: update document block data: {}",
              err
            ))
          })?;
        }
