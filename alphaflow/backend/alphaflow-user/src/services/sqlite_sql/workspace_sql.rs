...
use alphaflow_sqlite::{query_dsl::*, ExpressionMethods};
...
) -> Result<Vec<UserWorkspace>, AlphaflowError> {
...
#[allow(dead_code)]
fn delete_existing_workspaces(uid: i64, conn: &mut SqliteConnection) -> Result<(), AlphaflowError> {
  diesel::delete(
...
impl TryFrom<(i64, &UserWorkspace)> for UserWorkspaceTable {
  type Error = AlphaflowError;

  fn try_from(value: (i64, &UserWorkspace)) -> Result<Self, Self::Error> {
    if value.1.id.is_empty() {
      return Err(AlphaflowError::new(ErrorCode::InvalidData, "The id is empty"));
    }
    if value.1.workspace_database_id.is_empty() {
      return Err(AlphaflowError::new(ErrorCode::InvalidData, "The database storage id is empty"));
    }
