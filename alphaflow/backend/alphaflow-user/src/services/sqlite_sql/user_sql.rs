...
use alphaflow_error::{internal_error, AlphaflowError};
use std::str::FromStr;

use alphaflow_user_pub::cloud::UserUpdate;
...
pub fn select_user_profile(uid: i64, mut conn: DBConnection) -> Result<UserProfile, AlphaflowError> {
  let user: UserProfile = user_table::dsl::user_table
    .filter(user_table::id.eq(&uid.to_string()))
    .first::<UserTable>(&mut *conn)
    .map_err(|err| {
      AlphaflowError::new(ErrorCode::RecordNotFound, format!(
...
pub(crate) fn vacuum_database(mut conn: DBConnection) -> Result<(), AlphaflowError> {
  sql_query("VACUUM")
    .execute(&mut *conn)
