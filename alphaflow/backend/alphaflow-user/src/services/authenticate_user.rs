...
use alphaflow_error::{internal_error, ErrorCode, AlphaflowError, AlphaflowResult};
use alphaflow_sqlite::kv::KVStorePreferences;
use alphaflow_sqlite::DBConnection;
use alphaflow_user_pub::entities::UserWorkspace;
...
impl AuthenticateUser {
  ...
  pub fn user_id(&self) -> AlphaflowResult<i64> {
    let session = self.get_session()?;
    Ok(session.user_id)
  }

  pub fn device_id(&self) -> AlphaflowResult<String> {
    Ok(self.user_config.device_id.to_string())
  }

  pub fn workspace_id(&self) -> AlphaflowResult<String> {
    let session = self.get_session()?;
    Ok(session.user_workspace.id.clone())
  }

  pub fn workspace_database_object_id(&self) -> AlphaflowResult<String> {
    let session = self.get_session()?;
    Ok(session.user_workspace.workspace_database_id.clone())
  }

  pub fn get_collab_db(&self, uid: i64) -> AlphaflowResult<Weak<CollabKVDB>> {
    self
      .database
      .get_collab_db(uid)
  ...
  pub fn get_sqlite_connection(&self, uid: i64) -> AlphaflowResult<DBConnection> {
  ...
  pub fn get_user_data_dir(&self) -> AlphaflowResult<PathBuf> {
  ...
  pub fn close_db(&self) -> AlphaflowResult<()> {
    let session = self.get_session()?;
  ...
  pub fn is_collab_on_disk(&self, uid: i64, object_id: &str) -> AlphaflowResult<bool> {
    let session = self.get_session()?;
  ...
  pub fn set_session(&self, session: Option<Arc<Session>>) -> Result<(), AlphaflowError> {
    match session {
      None => {
  ...
  pub fn set_user_workspace(&self, user_workspace: UserWorkspace) -> AlphaflowResult<()> {
    let session = self.get_session()?;
