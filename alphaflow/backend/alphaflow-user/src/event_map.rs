...
use alphaflow_derive::{Alphaflow_Event, ProtoBuf_Enum};
...
pub fn init(user_manager: Weak<UserManager>) -> AFPlugin {
  ...
  AFPlugin::new()
    ...
    .event(UserEvent::ImportAlphaFlowDataFolder, import_alphaflow_data_folder_handler)
...
#[derive(Clone, Copy, PartialEq, Eq, Debug, Display, Hash, ProtoBuf_Enum, Alphaflow_Event)]
#[event_err = "AlphaflowError"]
pub enum UserEvent {
  ...
  #[event(input = "ImportAlphaFlowDataPB")]
  ImportAlphaFlowDataFolder = 41,
...
pub trait UserStatusCallback: Send + Sync + 'static {
  ...
  async fn did_expired(&self, _token: &str, _user_id: i64) -> AlphaflowResult<()> {
    Ok(())
  }
