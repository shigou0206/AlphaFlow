...
use crate::{Arcvar, ErrorReason, JmespathError};
...
impl<'a> Lexer<'a> {
    ...
    fn consume_negative_number(&mut self, pos: usize) -> Result<Token, JmespathError> {
        ...
        match self.iter.next() {
            Some((_, c)) if c.is_numeric() && c != '0' => {
                Ok(self.consume_number(pos, c, true)?)
            }
            _ => {
    ...
    fn consume_quoted_identifier(&mut self, pos: usize) -> Result<Token, JmespathError> {
        self.consume_inside(pos, '"', |s| {
            ...
            match Variable::from_json(format!(r##""{}""##, s).as_ref()) {
                ...
                Ok(j) => Ok(QuotedIdentifier(
                    j.as_string().cloned().ok_or_else(|| {
    ...
    fn consume_raw_string(&mut self, pos: usize) -> Result<Token, JmespathError> {
        ...
        self.consume_inside(pos, '\'', |s| {
            Ok(Literal(Arcvar::new(Variable::String(s.replace("\\'", "'")))))
        })
    }
...
mod tests {
    ...
    fn tokenize_raw_string_test() {
        assert_eq!(
            tokenize_queue("'foo'"),
            vec![
                (0, Literal(Arcvar::new(Variable::String("foo".to_string())))),
                (5, Eof)
            ]
        );
        assert_eq!(
            tokenize_queue("''"),
            vec![
                (0, Literal(Arcvar::new(Variable::String("".to_string())))),
                (2, Eof)
            ]
        );
        assert_eq!(
            ...
            vec![
                (
                    0,
                    Literal(Arcvar::new(Variable::String("a\\nb".to_string())))
                ),
                (6, Eof)
            ]
        );
    ...
    fn tokenize_literal_test() {
        ...
        assert_eq!(
            tokenize_queue("`\"a\"`"),
            vec![
                (0, Literal(Arcvar::new(Variable::String("a".to_string())))),
                (5, Eof)
            ]
        );
        assert_eq!(
            tokenize_queue("`\"a b\"`"),
            vec![
                (0, Literal(Arcvar::new(Variable::String("a b".to_string())))),
                (7, Eof)
            ]
        );
    }
    ...
    fn tokenize_successive_test() {
        ...
        assert_eq!(
            tokens[4],
            (11, Literal(Arcvar::new(Variable::String("a".to_string()))))
        );
