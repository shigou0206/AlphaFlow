...
use crate::{Arcvar, JmespathError};
    ...
    fn eq(&self, other: &Variable) -> bool {
        if self.get_type() != other.get_type() {
            ...
            match self {
                ...
                // 这里 now `Expref(Box<Ast>)`
                Variable::Expref(ref e) => {
                    // other 也应当是 `Expref(Box<Ast>)`, so match
                    if let Variable::Expref(ref e2) = other {
                        // 你可以写 e == e2 (如果 Ast: PartialEq)
                        // e 及 e2 是 Box<Ast>, so e == e2 => *e == *e2
                        *e == *e2
                    } else {
                        false
                    }
...
{
    let mut map: BTreeMap<String, Arcvar> = BTreeMap::new();
    for kvp in value {
...
impl Variable {
    ...
    pub fn as_array(&self) -> Option<&Vec<Arcvar>> {
        match self {
    ...
    /// Returns None otherwise.
    pub fn as_object(&self) -> Option<&BTreeMap<String, Arcvar>> {
        match self {
    ...
    #[inline]
    pub fn get_field(&self, key: &str) -> Arcvar {
    ...
    #[inline]
    pub fn get_index(&self, index: usize) -> Arcvar {
    ...
    pub fn get_negative_index(&self, index: usize) -> Arcvar {
        if let Variable::Array(ref array) = self {
    ...
    pub fn slice(&self, start: Option<i32>, stop: Option<i32>, step: i32) -> Option<Vec<Arcvar>> {
...
fn slice(array: &[Arcvar], start: Option<i32>, stop: Option<i32>, step: i32) -> Vec<Arcvar> {
    let mut result = vec![];
...
// for Variable and Arcvar. There are a few instances of
...
struct MapDeserializer {
    iter: <BTreeMap<String, Arcvar> as IntoIterator>::IntoIter,
...
#[doc(hidden)]
pub struct SeqState(Vec<Arcvar>);
...
impl ser::Serializer for Serializer {
    ...
    fn serialize_bytes(self, value: &[u8]) -> Result<Variable, Error> {
        let vec = value
            .iter()
            .map(|&b| Arcvar::new(Variable::Number(Number::from(b))))
            .collect();
    ...
    {
        let mut values = BTreeMap::new();
        values.insert(String::from(variant), Arcvar::new(to_variable(&value)?));
        Ok(Variable::Object(values))
    }
...
impl ser::SerializeSeq for SeqState {
    ...
    where
        T: ser::Serialize,
    {
        self.0.push(Arcvar::new(to_variable(value)?));
        Ok(())
    }
...
impl ser::SerializeTupleVariant for TupleVariantState {
    ...
    where
        T: ser::Serialize,
    {
        self.vec.push(Arcvar::new(to_variable(&value)?));
        Ok(())
    }

    fn end(self) -> Result<Variable, Error> {
        let mut object = BTreeMap::new();
        object.insert(self.name, Arcvar::new(Variable::Array(self.vec)));
...
impl ser::SerializeMap for MapState {
        ...
        T: ser::Serialize,
    {
        ...
        self.map.insert(key, Arcvar::new(to_variable(value)?));
        Ok(())
    }
...
impl ser::SerializeStructVariant for StructVariantState {
    ...
    {
        self.map
            .insert(String::from(key), Arcvar::new(to_variable(value)?));
        Ok(())
    }

    fn end(self) -> Result<Variable, Error> {
        let mut object = BTreeMap::new();
        object.insert(self.name, Arcvar::new(Variable::Object(self.map)));
        Ok(Variable::Object(object))
    }
}
...
mod tests {
    ...
    fn gets_value_from_object() {
        ...
        assert_eq!(
            Arcvar::new(Variable::Number(Number::from_f64(1.0).unwrap())),
            var.get_field("foo")
        );
    }
    ...
    fn test_parses_json_object() {
        ...
        sub_obj.insert("c".to_string(), Arcvar::new(Variable::Bool(true)));
        expected.insert("b".to_string(), Arcvar::new(Variable::Object(sub_obj)));
    ...
    fn test_converts_to_json() {
        ...
        let round_trip = serde_json::from_value::<Variable>(val).unwrap();
        assert_eq!(
            Variable::Array(vec![Arcvar::new(Variable::String("a".to_string()))]),
            round_trip
        );
    }
