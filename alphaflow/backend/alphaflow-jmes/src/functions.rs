...
use crate::{Context, ErrorReason, JmespathError, Arcvar, RuntimeError};
use serde_json::Number;

/// Represents a JMESPath function.
pub trait Function: Sync + Send {
    /// Evaluates the function against an in-memory variable.
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult;
}
...
impl ArgumentType {
    ...
    pub fn is_valid(&self, value: &Arcvar) -> bool {
...
pub struct CustomFunction {
    ...
    f: Box<dyn Fn(&[Arcvar], &mut Context<'_>) -> SearchResult + Sync + Send>,
}
...
where
    F: Send + Sync + Fn(&[Arcvar], &mut Context<'_>) -> SearchResult,
{
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        (self)(args, ctx)
    }
...
impl Signature {
    ...
    pub fn validate(&self, args: &[Arcvar], ctx: &Context<'_>) -> Result<(), JmespathError> {
...
macro_rules! min_and_max_by {
    ($ctx:expr, $operator:ident, $args:expr) => {{
        ...
        if vals.is_empty() {
            return Ok(Arcvar::new(Variable::Null));
        }
...
impl Function for AbsFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        self.signature.validate(args, ctx)?;
        match args[0].as_ref() {
            Variable::Number(n) => Ok(Arcvar::new(Variable::Number(
                Number::from_f64(
                    n.as_f64()
                        .ok_or_else(|| {
                            JmespathError::new(
...
impl Function for ContainsFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        let needle = &args[1];
        match **haystack {
            Variable::Array(ref a) => Ok(Arcvar::new(Variable::Bool(a.contains(needle)))),
            Variable::String(ref subj) => match needle.as_string() {
                None => Ok(Arcvar::new(Variable::Bool(false))),
                Some(s) => Ok(Arcvar::new(Variable::Bool(subj.contains(s)))),
            },
            _ => unreachable!(),
        }
    }
...
impl Function for EndsWithFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        Ok(Arcvar::new(Variable::Bool(subject.ends_with(search))))
    }
}
...
impl Function for JoinFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        let result = values
            ...
            .join(glue);
        Ok(Arcvar::new(Variable::String(result)))
    }
}
...
impl Function for KeysFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        let keys = object
            .keys()
            .map(|k| Arcvar::new(Variable::String((*k).clone())))
            .collect::<Vec<Arcvar>>();
        Ok(Arcvar::new(Variable::Array(keys)))
    }
}
...
impl Function for LengthFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        self.signature.validate(args, ctx)?;
        match args[0].as_ref() {
            Variable::Array(ref a) => Ok(Arcvar::new(Variable::Number(Number::from(a.len())))),
            Variable::Object(ref m) => Ok(Arcvar::new(Variable::Number(Number::from(m.len())))),
            // Note that we need to count the code points not the number of unicode characters
            Variable::String(ref s) => Ok(Arcvar::new(Variable::Number(Number::from(
                s.chars().count(),
            )))),
            _ => unreachable!(),
        }
...
impl Function for MapFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        Ok(Arcvar::new(Variable::Array(results)))
    }
}
...
impl Function for MergeFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        Ok(Arcvar::new(Variable::Object(result)))
    }
}
    ...
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        if args[0].is_array() {
            let mut values = args[0]
                ...
                .clone();
            values.reverse();
            Ok(Arcvar::new(Variable::Array(values)))
        } else {
            let word: String = args[0]
                ...
                .rev()
                .collect();
            Ok(Arcvar::new(Variable::String(word)))
        }
    }
}
...
impl Function for SortByFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        let vals = args[0]
            ...
            .clone();
        if vals.is_empty() {
            return Ok(Arcvar::new(Variable::Array(vals)));
        }
        let ast = args[1].as_expref().ok_or_else(|| {
            JmespathError::new(
                "",
        ...
        let mut mapped: Vec<(Arcvar, Arcvar)> = vec![];
        ...
        Ok(Arcvar::new(Variable::Array(result)))
    }
}
...
impl Function for StartsWithFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        Ok(Arcvar::new(Variable::Bool(subject.starts_with(search))))
    }
}
...
impl Function for ToArrayFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        match *args[0] {
            ...
            _ => Ok(Arcvar::new(Variable::Array(vec![args[0].clone()]))),
        }
    }
}
    ...
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        match *args[0] {
            ...
            Variable::String(ref s) => match Variable::from_json(s) {
                Ok(f) => Ok(Arcvar::new(f)),
                Err(_) => Ok(Arcvar::new(Variable::Null)),
            },
...
impl Function for ToStringFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        match *args[0] {
            ...
            _ => Ok(Arcvar::new(Variable::String(args[0].to_string()))),
        }
    }
}
...
impl Function for TypeFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        self.signature.validate(args, ctx)?;
        Ok(Arcvar::new(Variable::String(args[0].get_type().to_string())))
    }
}
...
impl Function for ValuesFn {
    fn evaluate(&self, args: &[Arcvar], ctx: &mut Context<'_>) -> SearchResult {
        ...
        Ok(Arcvar::new(Variable::Array(
            map.values().cloned().collect::<Vec<Arcvar>>(),
        )))
    }
}