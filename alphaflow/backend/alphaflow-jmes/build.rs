...
"##,
    ...
    if bench_type == "interpret" || bench_type == "full" {
        ...
        // 原先这里是 let data = Rcvar::new( ... ) => 改成 Arcvar::new( ... )
        f.write_all(
            format!(
                r##"

fn {}(b: &mut Bencher) {{
    let data = Arcvar::new(Variable::from_json({:?}).expect("Invalid JSON given"));
...
fn test_{}() {{
    ...
    let data = Arcvar::new(Variable::from_json({:?}).unwrap());
