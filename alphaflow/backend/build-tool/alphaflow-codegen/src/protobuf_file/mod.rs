...
fn generate_dart_protobuf_files(
  ...
  if std::env::var("FLUTTER_ALPHA_SDK_PATH").is_err() {
    log::error!("FLUTTER_ALPHA_SDK_PATH was not set, skip generate dart pb");
    return;
  }

  let mut output = PathBuf::new();
  output.push(std::env::var("CARGO_MAKE_WORKING_DIRECTORY").unwrap());
  output.push(std::env::var("FLUTTER_ALPHA_SDK_PATH").unwrap());
  output.push("lib");
