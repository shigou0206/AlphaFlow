...
pub fn get_ast_enums(ast: &syn::File) -> Vec<AlphaflowEnum> {
  let mut alphaflow_enums: Vec<AlphaflowEnum> = vec![];
  let ast_result = ASTResult::new();
  ...
  ast.items.iter().for_each(|item| {
    ...
    if let Item::Enum(item_enum) = item {
      ...
      alphaflow_enums.push(AlphaflowEnum {
