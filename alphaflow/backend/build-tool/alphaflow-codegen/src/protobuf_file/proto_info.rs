#![allow(dead_code)]
use crate::alphaflow_toml::{parse_crate_config_from, CrateConfig, AlphaflowConfig};
use crate::util::*;
...
pub struct ProtobufCrate {
  ...
  alphaflow_config: AlphaflowConfig,
}
...
impl ProtobufCrate {
  pub fn from_config(config: CrateConfig) -> Self {
    ProtobufCrate {
      ...
      alphaflow_config: config.alphaflow_config,
    }
  }
  ...
  pub fn protobuf_crate_path(&self) -> PathBuf {
    let crate_path = PathBuf::from(&self.alphaflow_config.protobuf_crate_path);
  ...
  pub fn proto_output_path(&self) -> PathBuf {
    let output_dir = PathBuf::from(&self.alphaflow_config.proto_output);
