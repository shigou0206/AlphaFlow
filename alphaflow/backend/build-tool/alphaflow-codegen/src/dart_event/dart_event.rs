...
use alphaflow_ast::ctxt::ASTResult;

use crate::ast::EventASTContext;
use crate::alphaflow_toml::{parse_crate_config_from, CrateConfig};
...
pub fn gen(crate_name: &str) {
  ...
  if std::env::var("FLUTTER_ALPHAFLOW_SDK_PATH").is_err() {
    println!("FLUTTER_ALPHAFLOW_SDK_PATH was not set, skip generate dart pb");
    return;
  }
  ...
  let dart_event_folder: PathBuf = [
    ...
    &std::env::var("FLUTTER_ALPHAFLOW_SDK_PATH").unwrap(),
    "lib",
    "dispatch",
    "dart_event",
    crate_name,
...
impl DartEventCrate {
  pub fn from_config(config: &CrateConfig) -> Self {
    DartEventCrate {
      ...
      event_files: config.alphaflow_config.event_files.clone(),
    }
  }
}
      ...
      ast
        ...
        .flat_map(|item| match item {
          Item::Enum(item_enum) => {
            ...
            let attrs = alphaflow_ast::enum_from_ast(
              &ast_result,
              &item_enum.ident,
              &item_enum.variants,
              &item_enum.attrs,
